...
app.get("/", (req, res) => {
  try {
    res.status(200).json("welcome on Deliveroo backend API");
  } catch (error) {
...
// get data's deliveroo with axios from API reacteur
app.get("/api_deliveroo", async (req, res) => {
  try {
    const bearerToken = process.env.BEARER_TOKEN;
    // wait from response reacteur with token in headers
